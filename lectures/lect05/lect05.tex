...
To a Multiple Layer Perceptron (MLP) \footnote{Although, we call it
  ...
  (continous nonlinearities), instead of step-wise discontinous
...
\begin{titledslide}{Computing partial derivatives 1}
  ...
  \item We will associate the \emph{bias} for a unit with a dummy
    input fixed to 1.
  \item It computes a value $z_j$ by first computing $a_j$, a weighted sum of its
    inputs (from the prevous layer), and then sending $a_j$ to some nonlinear
...
\begin{titledslide}{Computing partial derivatives 3}
  ...
  \end{equation}
  \begin{itemize}
  \item For the hidden units we use the chain rule:
  \end{itemize}
  \[
...
\begin{titledslide}{Error Backpropagation}
  ...
  \item Use (\ref{eq:dz}) to evalute the required derivatives.
  \end{enumerate}
