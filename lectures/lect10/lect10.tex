...
\item A Markov chain defines a sequence of marginal distributions; for
  the BN above these are $P(x_{1})$, $P(x_{2})$, $P(x_{3})$ and
  $P(x_{4})$.
...
\item The sampled values we draw are \textbf{not} independent, but
  this is a price we have to pay.
\end{itemize}
...
%  \item Let $p(\mathbf{z})$ be the \emph{target distribution}.
  ...
  \item Let the current state be $\mathbf{z}^{(\tau)}$. When using the
    ...
    \item We then accept $\mathbf{z}^{*}$ as the new state with a certain
      \emph{acceptance probability}.
    If we don't accept $\mathbf{z}^{*}$ then we `stay
      where we are', so that $\mathbf{z}^{(\tau)}$ is both the old
      and new state.
    \end{enumerate}
  \end{itemize}
...
\begin{titledslide}{Does Metropolis-Hastings (always) work?}.
  ...
  \item The \emph{rate} at which we converge to the target
    distribution is greatly influenced by the choice of proposal distribution.
  \end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
  ...
  \item If we choose a \emph{conjugate prior} for $P(\theta)$, then
    representing and computing $P(\theta|D=d)$ is easy.
  \item You have seen this with Rui in the context of (sequential)
