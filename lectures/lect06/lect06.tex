...
\begin{titledslide}{Non-saturating activation functions for deep learning}
  ...
  \item ReLU (and variants) is a popular choice. 
  \end{itemize}
...
\begin{titledslide}{Parameter initialisation}
  ...
  \item \dots i.e.\ the error `landscape' has many `valleys' and standard
    gradient descent just end up at the bottom of whichever valley it
    happens to start in.
...
\begin{titledslide}{Dropout}
  ...
  \begin{itemize}
  \item \emph{Dropout} is where we randomly turn of all outgoing
    connections from a unit with some given probability $p$.
