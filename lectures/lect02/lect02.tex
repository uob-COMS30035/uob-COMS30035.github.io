...
\begin{frame}[fragile]
  ...
  \centerline{\includegraphics[scale=0.2]{../figures/world_data.pdf}}
  \begin{itemize}
  \item ML attempts to learn \textbf{models} of the world
    \begin{itemize}
    \item Usually with many simplifications
    \item Models are typically a way of understanding how \textbf{input} data relates to the  \textbf{outputs}
...
\begin{frame}[fragile]
  ...
  \item ML attempts to \textbf{learn} models of the world
    ...
    \end{itemize}
  \uncover<3->{
  \item The type of data available, and its size, sets limits on which forms
    of learning we can use.} 
  \end{itemize}
...
\begin{frame}[fragile]
  ...
  %\centerline{\includegraphics[scale=0.5]{../figures/unsupervised_learning.pdf}}
    ...
    \item Density estimation (mixture models,  language models):  used
      to estimate the probability distributions from data \vspace{3pt}	
    \end{itemize}
...
\begin{frame}[fragile]
  ...
  Each input is paired with an output -- a ``label'' or ``target'' --
  and the model is trained to minimise the error (difference) between
  its output and the target. Sometimes we minimise the error plus a complexity penalty.%\vspace{5pt}
  %\centerline{\includegraphics[scale=0.45]{../figures/supervised_learning.pdf}}
  
  %\uncover<2->{
    Example tasks:
    \begin{itemize}
    \item Regression: continuous outputs, typically numbers (e.g.\  air temperatures over time)
    \item Classification: discrete outputs, typically category labels (e.g.\  dog or bagel?) \vspace{3pt}
    \end{itemize}
  %}
  %\uncover<3->{
...
\begin{frame}[fragile]
  ...
  field on its own -- \emph{we do \textbf{not} teach RL on this
    unit}.\vspace{5pt}
  \centerline{\includegraphics[scale=0.5]{../figures/reinforcement_learning.pdf}}
...
\begin{frame}[fragile]
  ...
  \centerline{\uncover<2->{\includegraphics[scale=0.28]{../figures/under_overfitting_1.png}
      \hspace{6pt}}
    \uncover<3->{\includegraphics[scale=0.28]{../figures/under_overfitting_2.png}
      \hspace{6pt}}
    \uncover<4->{\includegraphics[scale=0.28]{../figures/under_overfitting_3.png}}}
...
\begin{frame}[fragile]
  ...
  A common method is to \emph{split the dataset}. Let's look again at the data used in the previous slide\\
  \vspace{10pt}
  \centerline{\includegraphics[scale=0.5]{../figures/model_selection_1.png}}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]

  \frametitle{Model selection} Let's split the full dataset
  into:\vspace{5pt}
\begin{itemize}
...
\end{itemize}
\vspace{10pt}
\centerline{\includegraphics[scale=0.5]{../figures/model_selection_2.png} \hspace{6pt} \uncover<2->{\includegraphics[scale=0.5]{../figures/model_selection_3.png}}}
...
\begin{frame}[fragile]
  ...
  \item \textbf{Testing dataset}: Used to test the model for general
    fitting quality after the optimisation procedure (i.e.\ the
    machine learning) has
    finished \begin{small}(e.g.\ use 10\% of the full
      dataset)\end{small}.
\end{itemize}
\vspace{10pt}
\centerline{\includegraphics[scale=0.5]{../figures/model_selection_4.png}}
...
\end{itemize}
\vspace{10pt}
\centerline{\includegraphics[scale=0.4]{../figures/cross_validation.png}}
...
\begin{frame}[fragile]
  ...
  \item \begin{small}If we were to divide the space into cells we
    ...
    \uncover<2->{\item However, its often possible to find effective
      ...
      \end{itemize}}
  \end{itemize}
  \centerline{\includegraphics[scale=0.15]{../figures/curve_dim.png}}
...
\uncover<2->{\begin{itemize}
	...
	\item \emph{But} there is no universally best model selection procedure-- \textbf{no free lunch theorem} (Wolpert 1996).
	\item \uncover<3->{Why? Models always make assumptions and
            these often do not generalise across domains -- different
            domains need different models.}
          \item \uncover<4->{No free lunch results are essentially a
              reinvention of the \emph{problem of induction} studied
              in philosophy
            \item `` \dots happenings which depend upon an infinite
              number of cases cannot be determined by a finite number
              of experiments \dots'' (Leibniz in a 1703 letter to Bernoulli).}
\end{itemize}}
...
\begin{frame}[fragile]
  ...
  \end{itemize}
  \centerline{\uncover<1->{\includegraphics[scale=0.45]{../figures/parametric_model.png}
      \hspace{6pt}}
    \uncover<2->{\includegraphics[scale=0.45]{../figures/nonparametric_model.png}
      \hspace{6pt}}}
...
\begin{frame}[fragile]
  ...
  \centerline{\uncover<2->{\includegraphics[scale=0.4]{../figures/boston_crimerate_lessdata.pdf}
      \hspace{6pt}}
    \uncover<3->{\includegraphics[scale=0.4]{../figures/boston_crimerate.pdf}}}
...
\begin{titledslide}{Reading}
  ...
  \begin{itemize}
  \item Bishop \S1.1--\S1.4. (\S1.2 is about Probability Theory which
    we did not cover here but is useful `revision'.)
  \item The whole of Murphy Chapter~1 is a good read, but if you wish
    to just focus on this lecture's material look at Murphy \S1.1-\S1.2.
  \end{itemize}
...
\begin{titledslide}{Problems and quizzes}
  ...
  \item Quizzes:
    \begin{itemize}
    \item Week~1: Machine Learning Concepts 
    \end{itemize}
  \end{itemize}
