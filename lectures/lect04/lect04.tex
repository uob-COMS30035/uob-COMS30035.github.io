...
\uncover<4->{\item Optimisation: least-squares (as for regression) \footnote{See Bishop p184 and p190.}, where we want to minimise the cost or error function:
\end{itemize}
$\boxed{E = \frac{1}{2}\sum^N_{n=1} (\bm{w}^T\bm{x}_n + w_0 - t_n)^2}$ \small where $t_n$ are the targets/labels (e.g. $t_1$ = $C_1$).}
\end{frame}
...
\uncover<4->{	\item And in turn to $\mathlarger{\sum_{n=1}^N}\{ y_n - t_n \} x_n$ \footnote{You can find the full derivation \href{https://uob-coms30035.github.io/Material/CrossEntropy.html}{here}.}}
\end{enumerate}
