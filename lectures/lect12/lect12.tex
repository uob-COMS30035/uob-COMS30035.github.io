...
\begin{titledslide}{MLE for a Gaussian mixture}
  ...
  \item So have to resort to an iterative algorithm where we are only guaranteed a local maximum.
    \item The algorithm is called the \emph{Expectation-Maximization (EM) algorithm}.
...
\begin{itemize}
\item EM is used to (try to) maximise log-likelihood functions of the following form:
\end{itemize}

\end{titledslide} 

\begin{titledslide}{The General EM algorithm}

  \textbf{E step} Evaluate $p(\Zvec|\Xvec,\thetavec^{\mbox{old}})$ so we have:
  \[
     Q(\thetavec|\thetavec^{\mbox{old}}) = \sum_{\zvec} p(\Zvec|\Xvec, \thetavec^{\mbox{old}}) \ln p(\Xvec, \Zvec | \thetavec)
  \]
...
\thetavec^{\mbox{new}}
= 
\arg\max_{\thetavec} Q(\thetavec|\thetavec^{\mbox{old}})
\]

%\begin{itemize}
%\item Computing $p(\Zvec|\Xvec,\thetavec^{\mbox{old}})$ `in full' often not necessary. Computing expected values with respect to this distribution often enough (hence the name: E step)
%\item Just need to know enough about $p(\Zvec|\Xvec,\thetavec^{\mbox{old}})$ to do the M step
%\end{itemize}
