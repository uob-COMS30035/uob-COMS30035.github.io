...
\begin{titledslide}{Acknowledgement}
  ...
  \item These slides are adapted from ones originally created by
    \href{https://www.dpag.ox.ac.uk/team/rui-ponte-costa}{Rui Ponte
      Costa} and later edited by Edwin Simpson, James Cussens and
    Xiyue Zhang. 
  \end{itemize}
...
\begin{itemize}
	\item The very beginning of neural network models in ML! \vspace{3pt}\\
	\item Directly inspired by how neurons process information: \vspace{3pt}\\	
...
\item We aim to minimise the following error $ -
  \mathlarger{\sum_{n \in {\cal M}} \bm{w}^T\phi_nt_n }$, where $\cal
  M$ denotes the misclassified datapoints and $\phi_{n} = \phi({\bm x}_{n})$.}
\end{itemize}
...
\centerline{\includegraphics[width=\textwidth]{../figures/nn.png}}

If we use a sigmoidal output unit \emph{activation function} then the
overall network function would be:
\[
  y_{k}({\bm x},{\bm w}) = \sigma\left(\sum_{j=1}^{M} w_{kj}^{(2)}
    h\left( \sum_{i=1}^{D}w_{ji}^{(1)}x_{i} + w_{j0}^{(1)}\right) + w_{k0}^{(2)}\right)
\]
...
\begin{itemize}
	...
	\item \textbf{Deep learning} refers to neural networks  with more than 1 hidden layer\\	
	\item They can be applied in any form of learning, but we will focus on supervised learning and classification in particular\\	
\uncover<2->{	\item Neural network recipe \footnote{Here we focus on simple feedforward nets but the recipe is the same for any neural network.}:
	\begin{itemize}	
...
Assume just one hidden layer and $\sigma$ for the output activation function\dots

\begin{enumerate}
	\item \small Calculate activations of the hidden layer: $a_j = \mathlarger{\sum_{i=1}^D} w_{ji}^{(1)}x_i + w_{j0}^{(1)}$  [linear]
	\item \small Pass it through a nonlinear function: $z_j = \sigma(a_j)$ [nonlinear]
\uncover<2->{	\item \small Calculate activations of the output layer: $a_k = \mathlarger{\sum_{j=1}^{M}} w_{kj}^{(2)}z_j + w_{k0}^{(2)}$  [linear]
	\item \small Compute predictions using a sigmoid: $y_k = \sigma(a_k)$ [nonlinear]}
\uncover<3->{	\item \small All together: $y_k = \sigma \left( \mathlarger{\sum_{j=1}^M} w_{kj}^{(2)}  h \left(\mathlarger{\sum_{i=1}^D} w_{ji}^{(1)}x_i + w_{j0}^{(1)}\right) + w_{k0}^{(2)} \right)$}
\end{enumerate}
...
\uncover<2->{In our neural network (one hidden layer) this means to update the weights using:

\begin{itemize}
	\item \small Output: $w_{kj}^{new} = w_{kj}^{old} - \Delta w_{kj}$, where $\Delta w_{kj} = h'(a_{k})(y_k - t_k)z_j$
	\item \small Hidden $w_{ji}^{new} = w_{ji}^{old} - \Delta w_{ji}$, where $\Delta w_{ji} = h'(a_{j}) \sum_{k}w_{kj}(y_k - t_k)h'(a_{k}) x_i$
