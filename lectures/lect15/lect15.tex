...
\item \textcolor{gray}{Mixtures of Experts}  
\end{itemize}
\end{frame}
...
\item \textcolor{gray}{Ensembles: Bagging
\item Ensembles: Boosting and Stacking}
\item \textcolor{gray}{Tree-based Models  
\item Mixtures of Experts} 
\end{itemize}
\end{frame}
...
\item \textcolor{gray}{Mixtures of Experts}  % this may be in the next "lecture"?
\end{itemize}
\end{frame}
...
\item Ensembles: Boosting and Stacking
\item Tree-based Models}  % decision trees effectively make the selection decisions on the fly; random forests then are ensembles. Maybe trees should be discussed before committees?
\item Mixtures of Experts  % this may be in the next "lecture"?
\end{itemize}
\end{frame}
...
\uncover<2->{\item Mixtures of experts: perform a soft, probabilistic split of the feature space $\rightarrow$ \emph{soft selection depending on features}}
%\emph{combine} models 
...
\item The prediction for the output target variable $y$ depends, i.e.\
  is \emph{conditional} on, the input feature vector $\bs x$
\item We will only be looking at one conditional mixture model called \emph{Mixture of Experts}
%\begin{itemize}
...
\end{equation}
\item Note that here, like Bishop, we are assuming that each component
  provides a distribution $p(t_n | \bs x_n, \bs\phi_k)$ over the
  target and the mixture thus also gives us a distribution over the
  target: $p(t_n | \bs x_n, \bs \phi, \bs\pi)$. We can use e.g.\ the
  mode of this distribution to provide a predicted value for the target.
...
\begin{titledslide}{Reading}
  ...
  \begin{itemize}
  \item Bishop Chapter 14, but you can skip: \S14.3.1, \S14.3.2, \S14.5.1, \S14.5.2.
