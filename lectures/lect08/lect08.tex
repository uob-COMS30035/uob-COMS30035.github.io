...
\begin{frame}[fragile]
\frametitle{Linear regression revisited}
Consider a linear regression model:
\begin{equation}
  \label{eq:lr}
  y(\xvec) = \params^{T}\feat(\xvec)
\end{equation}
where
\begin{enumerate}
...
\item $\feat(\xvec)$ is the $M$-dimensional feature vector which \xvec{} gets mapped to by the \emph{basis functions} \cite[\S3.1]{bishop06:_patter_recog_machin_learn}.
\end{enumerate}
\end{frame}
...
\item Let $N$ be the size of the data. Let $\designm$ be the
  ...
  we can reformulate in terms of an $N$-dimensional parameter vector
...
\end{equation}
\begin{itemize}
\item This is known as a \emph{dual representation}.
...
\begin{titledslide}{Soft margins}
  ...
  \item or even on the wrong side of the decicion boundary.
...
The red/blue areas in the plots on the last slide represente values of
